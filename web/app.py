    ...
    with one_at_a_time:
        ...
        try:
            ...
            job.result = {**cli.result_to_dict(job.bridge, job.loads, found, wind, results), "drawing": drawing(job.bridge, results),
                          ...
                          "replay": {girder: replay(job.bridge, results, girder) for girder in range(job.bridge.girders.count)}}
...
def max_moment_loads(bridge, results, girder):
    key = (girder, MAX_MOMENT, BIGGER_IS_WORSE)
...
def body_of(name, x_front_m, z_m, impact):
    ...
    # axles run behind the first one in +x, so a vehicle drives towards -x; its reverse is the same truck driving towards +x
    heading = 1 if name.endswith("_reversed") else -1
...
def drawing(bridge, results):
    ...
    for girder in range(bridge.girders.count):
        ...
        girders[girder] = {"at_m": at_m, "moment_kn_m": critical.response, "lane_reduction": critical.lane_reduction, "lane_pattern": critical.lane_pattern,
    ...
    return {"span_m": bridge.span_m, "width_m": bridge.width_m(), "skew": bridge.skew, "girder_lines_m": [float(z) for z in build_mesh(bridge).girder_lines_m],
            "strips": [{"name": s.name, "from_m": s.z_from_m, "to_m": s.z_to_m} for s in bridge.cross_section.strips], "max_moment": girders,
...
# a layout of vehicles rolled along the span as one: the girder moment from their wheels at every shift, off the influence surface
def rolling(bridge, surface, layout, lane_reduction):
    points = np.array([(x_m + dx, z_m + dz, load * impact * lane_reduction) for name, x_m, z_m, impact in layout
    ...
    heading = 1 if layout[0][0].endswith("_reversed") else -1
    enter_m, leave_m = bridge.span_m + margin_m - points[:, 0].min(), -margin_m - points[:, 0].max()
    shifts = np.union1d(np.linspace(leave_m, enter_m, ROLL_STEPS), [0.0])
    shifts = shifts[::-1] if heading < 0 else shifts
    moments = (points[:, 2] * surface.influence_at(points[None, :, 0] + shifts[:, None], points[None, :, 1])).sum(axis=1)
    return shifts, moments


# one rolled layout for the replay
def trial(bridge, surface, label, kind, layout, lane_reduction, total=None):
...
# the search replayed for one girder's maximum moment: each vehicle alone rolled along each lane, then setu's best lane arrangements
def replay(bridge, results, girder):
    key = (girder, MAX_MOMENT, BIGGER_IS_WORSE)
        ...
        for vehicle, from_edge_m in kinds:
            ...
            for z_m in sorted({round(nearest, 3), round((nearest + furthest) / 2, 3), round(furthest, 3)}):
                both = [trial(bridge, surface, f"{way.name.replace('Class_', '').replace('_', ' ').replace(' reversed', ' ↺')} alone · c/w {lane} · z {z_m:.2f} m",
                              "single", [(way.name, bridge.span_m / 2, z_m, impact_factor(way.name, bridge.span_m))], 1.0) for way in both_directions_of(vehicle)]
                trials.append(max(both, key=lambda t: t["peak"]))
    ranked = search_quietly(surface, bridge.cross_section, bridge.span_m, BIGGER_IS_WORSE, bridge.wearing_course_thickness_m)
    for rank in reversed(range(min(ARRANGEMENTS_REPLAYED, len(ranked)))):
        ...
        found = trial(bridge, surface, f"{'Critical' if rank == 0 else f'Arrangement #{rank + 1}'}: {case.lane_pattern.replace('_', ' ').replace('class', 'Class').replace('70r', '70R').replace('Class a', 'Class A')}", "arrangement",
                      layout, case.lane_reduction, total=case.response)
        found["winner"] = rank == 0
