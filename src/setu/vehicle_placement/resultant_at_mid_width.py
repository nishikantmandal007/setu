# The other transverse condition the code asks for: the same vehicles, slid until the
# resultant of their loads sits on the carriageway centreline.
#
# This condition can never govern. It is one position inside the set the sweep already
# searches, so the sweep either finds it or finds something worse. It is computed and
# reported anyway, because the code asks for both conditions, and because the gap between
# them is worth seeing - relying on the centred position alone under-predicts the load, by
# a margin that is not always small. That reasoning is subtle enough that someone will
# otherwise "fix" it into the comparison, so keep it in mind: the two are not competitors.
#
# Everything moves together. Each vehicle slides from where the arrangement packs it hard
# left to where it packs it hard right, all by the same fraction of the room available. The
# resultant moves linearly with that fraction, so the fraction that centres it is solved
# for directly rather than searched - one division, not a search.
...
# When the arrangement has no room to slide, this is how close its one fixed position must
# already be to the target, in metres, to count as centred.
ALREADY_CENTRED_TOLERANCE_M = 1e-6

# How far outside the legal 0 to 1 range the raw fraction may fall and still count as
# reaching the centreline - guards against a fraction that misses only by floating-point
# noise, before it gets clamped back into range.
...
class ResultantCentredPlacement:
    ...
    # False when clearances stopped the resultant from reaching the centreline.
    is_exactly_centred: bool


def weight_of(vehicle: Vehicle) -> float:
    # The vehicle's total unfactored load, in kilonewtons.
...
def centre_the_resultant(
    ...
    # Returns the worst resultant-centred placement on each carriageway - one per
    # carriageway, each the worst of that carriageway's admissible arrangements.
...
def centre_one_arrangement(
    ...
    fraction, is_exact = fraction_that_centres_the_resultant(
...
def how_far_each_vehicle_can_go(
    ...
    # Each vehicle's centreline packed hard left, and packed hard right.
...
def fraction_that_centres_the_resultant(
    ...
    # How far across to slide everything, as a fraction of the room available.
    #
    # The resultant moves linearly with that fraction, so the answer is one division rather
    # than a search. A fraction outside 0 to 1 means the clearances will not let the
    ...
    resultant_packed_left_m = (
        sum(
            weight_kn * left_m
            for weight_kn, left_m in zip(weights_kn, packed_left_m, strict=True)
        )
        / total_kn
    )
    how_far_it_can_move_m = (
        sum(
            weight_kn * (right_m - left_m)
            for weight_kn, left_m, right_m in zip(
                weights_kn, packed_left_m, packed_right_m, strict=True
            )
        )
        / total_kn
    )
    ...
    if abs(how_far_it_can_move_m) < TOLERANCE_M:
        return 0.0, abs(resultant_packed_left_m - target_m) < ALREADY_CENTRED_TOLERANCE_M

    fraction = (target_m - resultant_packed_left_m) / how_far_it_can_move_m
    is_exact = -FRACTION_TOLERANCE <= fraction <= 1.0 + FRACTION_TOLERANCE
    return min(max(fraction, 0.0), 1.0), is_exact


def representative_vehicle(block: str) -> Vehicle:
    # The vehicle whose weight stands for this block, for the resultant calculation alone -
    # a different question from which vehicles VEHICLES_ALLOWED_IN_BLOCK says the block may
    # actually carry. Picking one representative weight is enough for this purpose.
