# Working out what a plate girder's section is worth: area, where its neutral axis sits,
# how stiff it is about each axis, and how much it resists twist - all from the plate
# sizes, by the parallel axis theorem, worked out one plate at a time.
...
def girder_properties(section: PlateGirderSection) -> GirderProperties:
    ...
    # Area of each plate, and the section as a whole.
    ...
    # Neutral axis: the area-weighted average of the plate centres.
    neutral_axis_m = (
    ...
    # Izz, by the parallel axis theorem, plate by plate: each plate's own stiffness about
    # its own centre, plus its area times how far that centre is from the neutral axis.
    ...
    # Iyy. Sideways, every plate is centred on the girder's own centreline, so there is no
    # parallel axis term to add.
    ...
    # Torsion constant. An open section built of thin plates twists as the sum of its
    # plates.
    torsion_constant_m4 = (
