...
class LaneArrangement:
    # one way to fill a carriageway with Class A lanes and 70R zones
...
# Table 6 design lanes for this carriageway width
...
# is the carriageway wide enough to load at all
...
# gap between two Class A vehicles, closing up on narrow roads
...
# width of each block in the pattern
...
# gap between each pair of neighbouring blocks
...
# kerb clearance at the left and right ends of the pattern
...
# does the pattern fit this carriageway
...
# every 70R zone touches a kerb or other 70R zones, as Table 6A draws
...
# is this pattern one of the Table 6A drawings
...
# a LaneArrangement for one pattern
...
# pack the blocks hard left between the kerbs, or None if they don't fit
...
# nearest and furthest the vehicle centre can be inside its block
...
# Table 6A lane reduction for this many loaded lanes
...
# carriageways narrower than 5.3 m get the residual UDL
def needs_residual_udl(carriageway_width_m):
    return float(carriageway_width_m) < RESIDUAL_UDL_APPLIES_BELOW_M

# the carriageway strips a Class A lane leaves uncovered
...
# parts of from..to not covered by any of the given pieces
