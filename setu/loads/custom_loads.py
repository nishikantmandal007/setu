...
from setu.models.custom_load import AREA, LINE, POINT
...
def pieces_of(model, load):
    ...
    raise ValueError(f"a custom load is a {POINT}, {LINE} or {AREA} load, got {load.shape!r}")
...
def check_on_the_deck(model, along_m, z_m, load):
    ...
    if not on_it:
        raise ValueError(f"custom load {load.name!r} reaches ({along_m:.3f} m along, {z_m:.3f} m across), which is off the deck")
