...
def test_a_design_value_is_the_sum_of_its_shares(results):
    governing = results.girders[1][MAX_MOMENT][BASIC][BIGGER_IS_WORSE]
...
def test_sagging_is_governed_by_live_load_leading(results):
    ...
    assert results.girders[1][MAX_MOMENT][BASIC][BIGGER_IS_WORSE].combination.endswith("live leading")
...
def test_symmetric_deck_gives_mirror_girders_the_same_values(results):
    ...
    first = results.girders[0][MAX_MOMENT][BASIC][BIGGER_IS_WORSE].value
    last = results.girders[BRIDGE.girders.count - 1][MAX_MOMENT][BASIC][BIGGER_IS_WORSE].value

    assert first == pytest.approx(last, rel=1e-6)


def test_the_custom_group_only_shows_in_its_own_combination(results):
    by_limit_state = results.girders[0][MAX_MOMENT]
...
def test_the_governing_girder_is_reported(results):
    ...
    assert governing.value == max(results.girders[g][MAX_MOMENT][BASIC][BIGGER_IS_WORSE].value for g in results.girders)


def test_temperature_comes_back_as_stresses_and_bearing_movement(results):
    thermal = results.thermal

    for girder, by_kind in thermal["girders"].items():
        assert by_kind["positive difference"].slab_top_kpa < 0
        assert by_kind["reverse difference"].slab_top_kpa > 0
    assert thermal["girders"][0]["positive difference"].slab_width_m < thermal["girders"][2]["positive difference"].slab_width_m
...
def test_high_wind_keeps_live_load_off(results):
    ...
    for by_response in stormy.girders.values():
        for by_direction in by_response[MAX_MOMENT].values():
...
def test_seismic_is_symmetric_too(results):
    ...
    first = results.girders[0][MAX_MOMENT][SEISMIC_COMBINATION][BIGGER_IS_WORSE].value
    last = results.girders[BRIDGE.girders.count - 1][MAX_MOMENT][SEISMIC_COMBINATION][BIGGER_IS_WORSE].value
